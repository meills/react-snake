...
class Snakeboard extends React.Component {
    ...
    gameTick() {
        ...
        this.checkGameOver(newCol, newRow);
    ...
    checkGameOver(newCol, newRow) {

        if (newCol >= GRID_SIZE || newRow >= GRID_SIZE || newCol < GRID_SIZE || newRow < GRID_SIZE) {
            console.log("game over");
        }

        for (let i = 0; i < this.state.snake.tail.length; i++) {
            if (this.state.snake.tail[i].row === newRow && this.state.snake.tail[i].col === newCol) {
    ...
    render() {
        ...
        if (this.state.gameStart) {
            ...
            return ( <React.Fragment><div className='grid'> </div>
            <div className='start-screen'> </div></React.Fragment>
            );
        }
