...
class Grid extends React.Component {
    ...
    initGrid() {
        ...
        this.setState({food: {ro}});

        console.log(this.state.food);

        const head = this.getGridCenter();
        this.setState({ snake: { head: head }});

        for (let row = 0; row < this.state.rows; row++) {
          for (let col = 0; col < this.state.cols; col++) {
            const isFood = row === this.state.food.row && col === this.state.food.col;
            const isHead = row === this.state.snake.head.row && col === this.state.snake.head.col
