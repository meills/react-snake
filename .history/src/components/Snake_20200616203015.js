...
class Snake extends React.Component {
    ...
    gameTick() {
        ...
        this.setState({food: newFood, snake: { head: {row: newRow, col: newCol}, tail: tail}});
    ...
    checkGameOver() {
        ...
        if(this.state.snake.tail.length === 1) {
            if (row === this.state.snake.tail[0].row && col === this.state.snake.tail[0].col) {
                this.props.endGame();
                return true;
            }
        }
    ...
    render() {
        const gridItems = this.state.grid.map((grid) => {
            return <div key={grid.row.toString() + "," + grid.col.toString()} 
                        className={ (grid.isFood ? "grid-item is-food" : "grid-item id= \'food\'") 
